...
| P3 | Memory-budget definition: does the 4 GB include the ~300–600 MB CUDA context? **Decided: exclude it, state it explicitly, report it separately.** ⚠️ **Half-closed 2026-08-20.** `scripts/measure_cuda_context.py` measures it in stages (bare context, + pool, + real kernels) and refuses to publish from a card with more than one tenant or >10% baseline drift — which the dev box is, so its 69 MiB residual is stamped `trusted: false`. Needs one 30-second run on an idle T4. Urgent rather than tidy since D26: the ship config sits at 4.000 GiB of 4.00 and has no slack for it. | Phase 8 |
